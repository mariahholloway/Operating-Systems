//worked with amira recievd help from shane oliver 
#include <stdio.h>
#include <stdlib.h>
#include <sys/types.h>
#include <sys/ipc.h>
...
int  main(int  argc, char *argv[]) {
     ...
     if (pid < 0) {
        ...
        for (i = 0; i < 25; i++) {
               ...
               if (account <= 100) {              // if account balance is less or equal to 100
                    deposit = random()% 101;      // deposits a random number between 0-100
                    if (deposit % 2 == 0) {       // if the deposit is even
                         account += deposit;
                         printf("Dear Old Dad: Deposits $%d / Balance = $%d\n",
                                   deposit, account);
                    } else {                      // if the deposit is odd
                         printf("Dear Old Dad: Doesn't have any money to give\n");
                    }
               } else {                           // if the account balance > 100
